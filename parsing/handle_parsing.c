...
/*   Updated: 2025/06/26 10:28:56 by engiusep         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	for_redir(t_cmd *current_cmd, t_token *current_token)
{
	...
	{
		current_cmd->infile = ft_strndup(current_token->next->value,
				ft_strlen(current_token->next->value));
		if (!current_cmd->infile)
			return (-1);
		current_cmd->heredoc = 0;
	}
	else if (current_token->type == REDIR_OUT && current_token->next
		&& current_token->next->type == WORD)
	{
		current_cmd->outfile = ft_strndup(current_token->next->value,
				ft_strlen(current_token->next->value));
