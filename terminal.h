...
int								add_in_env(char **origin_env, char **dest_env,
									int *i);
char							*clean_str(char *str);

// exec
void							loop_exec_pipeline(t_cmd **cmd, int *in_fd,
									int *pipefd);
int								no_child_pipe(t_cmd *cmd, t_shell *shell,
									int *pipefd, int *in_fd);
...
void							handle_next_pipe(int *in_fd, t_cmd *cmd,
									int *pipefd);
...
int								signal_and_single_redirect(t_cmd *cmd,
									t_shell *shell, int heredoc_fd);
int								redirect_choice_pipe_outfile(t_cmd *cmd,
									int *in_fd, int *pipefd);
int								redirect_choice_single(t_cmd *cmd,
									int heredoc_fd);
...
int								loop_line_checker(char **line, t_shell *shell,
									char **new_line, int *i);
int								end_loop_heredoc(char **new_line, char **line,
									int *i);
int								expand_last_exit(char **new_line,
									t_shell *shell, int *i);
int								expand_heredoc(char **line, char **new_line,
									t_shell *shell, int *i);
char							*str_trim_nl(char *line);
int								is_stop_word(char *line, char *stop_word);
int								pipe_loop(t_shell *shell, t_cmd *cmd,
									int *in_fd, int *pipefd);
int								check_all_arg_for_heredoc(t_cmd *cmd,
									t_shell *shell);
int								exec_pipeline(t_cmd *cmd, int *pipefd,
									t_shell *shell, int *in_fd);
void							close_fd_exit(int *pipefd, int in_fd);

// lexer
int								in_quote_var_env(char **result, t_shell *shell,
									t_index_lexer *index, char *str);
int								check_lex_in_loop(char *str,
									t_index_lexer *index, char **result,
									t_shell *shell);
int								primary_check_quote(char *line, int *i);
int								handle_special_symbols(t_token **tokens_list,
									char *str, t_index_lexer *index);
int								primary_checker(char *line);
int								checker_flag_symbol(t_index_lexer *index,
									int flag_symbol, t_token *next,
									t_token *previous);
...
int								ft_pipe(t_token **tokens_list,
									t_index_lexer *index);
...
int								add_in_str(char **result, char *var_in_env,
									t_index_lexer *index);
int								dollar_var_env(char **result,
									t_index_lexer *index, char *str,
									t_shell *shell);
int								start_loop(char *str, t_index_lexer *index,
									char **result, t_shell *shell);
void							end_loop(char **result, char *str,
									t_index_lexer *index);
void							end_loop2(char **result, char *str,
									t_index_lexer *index);
void							after_dollar_checker(t_index_lexer *index,
									char **result, t_shell *shell);
int								env_var_checker(char *str);
int								check_quote(char *str);
